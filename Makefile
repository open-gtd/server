...
all: fmt lint vendor | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
	$Q [ -f main.go ] && cd $(BASE) && $(GO) build \
			-tags release \
			-ldflags '-X $(PACKAGE)/cmd.Version=$(VERSION) -X $(PACKAGE)/cmd.BuildDate=$(DATE)' \
			-o bin/$(NAME) main.go || echo "File main.go does not exist"
...
.PHONY: update-vendor
update-vendor: glide.yaml | $(BASE) ; $(info $(M) updating dependencies for $(VDR)…)
	@echo vdr: $(VDR)
	$Q cd $(BASE) && $(GLIDE) update -v $(VDR)
	@touch $@
